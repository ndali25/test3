%% Find the draws closest to the pointwise median IRF (Fry and Pagan):

% Fry and Pagan IRFs:
resp_median=prctile(candidateirf,50,4);
resp_median_cum=prctile(candidateirf_cum,50,4);

distance_med=zeros(n,n,hor,draws);
distance_med_cum=zeros(n,n,hor,draws);
   ...
   for k=1:draws
       ...
       for variable=1:n
           ...
           for shock=1:n
               ...
               distance_med_cum(variable,shock,h,k)=((candidateirf_cum(variable,shock,h,k)-resp_median_cum(variable,shock,h))/nanstd(squeeze(candidateirf_cum(variable,shock,h,:)))).^2;
           end
...
distance_cum=squeeze(nansum(sum(nansum(distance_med_cum(1:n,1:n,:,:),1),2),3));
...
% Select the n closest draws
n_selected_draws = 10;


% Find the n closest draws to the pointwise cumulative IRFs
[distance_min,solutions]=mink(distance_cum,n_selected_draws);

% Uncomment this if you would like to the untransformed IRFs rather than the cumulative IRFs
% [distance_min,solutions]=mink(distance,n_selected_draws);


% Store the IRFs for the top 10 draws
