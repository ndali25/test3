...
function [PI,BigA,Sigma,errornorm,fittednorm]=BVAR_sur(y,p,c,y0_bar,delta)

    [Traw,K]=size(y);
    
    T=Traw-p;

    [pi_hat,Y,X,~,~,err]=VAR_sur(y,p,c,y0_bar,delta); % ML = OLS estimate 
    
    S=err'*err; % SSE
    
    % Draw Sigma from IW ~ (S,v), where v = T-(K*p+1)-K-1 :
    
    Sigma=iwishrnd(S,T-size(X,2)-K-1); 
