function [xFS, VFS, VVFS, loglik] = runKF(y, A, C, Q, R, x_0, Sig_0,stringFS)
...
if strcmp(stringFS,'smoother')
    S = FIS(y,C,R,A,Q,S); % run the smoother
    xFS = S.AmT;
    VFS = S.PmT;
    VVFS = S.PmT_1;
else % run only the filter
    xFS = S.AmU;
    VFS = S.PmU;
    VVFS = [];
