function Xcond = VARcf_DKcks(X,p,beta,Su,nDraws)
...
if nDraws == 0 % point forecast
    ...
    [xsmooth] = runKF_DK(yinput', AA, CC, QQ, diag(ones(1,N)*1e-12), initx, initV,c1,c2);

    Xcond = [Xinit;xsmooth(1:N,:)'];
else
    ...
    for kg =1:nDraws
        ...
        ystar = yinput'-yplus;
        ahatstar = runKF_DK(ystar, AA, CC, QQ, diag(ones(1,N)*1e-12), zeros(size(initx)), initV,zeros(N,1),zeros(size(initx)));
        atilda = ahatstar+aplus;
        Xcond(:,:,kg) = [Xinit;atilda(1:N,:)'];
    end
end
