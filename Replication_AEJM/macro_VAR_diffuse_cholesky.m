...
for i=1:hor
    candidateirf(:,:,i,k)=C(:,:,i,k)*S; % Cholesky IRFs
end
...
%% Figure 3.(b): All deterministic components (DC)
...
filename = strcat(saveFolder,'/Fig3b');
print(hFig, filename, '-dpdf','-r0')

%% Figure A-1.(b): IRF for Cholesky identification
...
filename = strcat(saveFolder,'/FigA1b');
print(hFig, filename, '-dpdf','-r0')
...
% set(hFig,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
% 
% filename = strcat(saveFolder,'/FigA2b_HD_FP3_',priorName);
% print(hFig, filename, '-dpdf','-r0')
